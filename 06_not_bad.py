...
def not_bad(s):
    ...
    if s.find('bad') > s.find('not'):
        if s.find('!') > s.find('bad'):
            return f'{s[:s.find("not")]}{"good"}{s[-1:]}'
        else:
            return f'{s[:s.find("not")]}{"good"}'    
    else:
        return s
    
   # print('{}{} {} '.format(test[:test.find("not")],'gooppd',test[-1:]))
    
    """
    if "bad" or "not" in s:
        return s
    elif s.index("bad") > s.index("not"):
        print(s.index("not"))
        print(s.index("bad"))
        return f'{s[:s.index("not")]}{"good!"}'
    return s
    """
